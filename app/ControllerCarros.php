...
public function getById($id) {
    try {
        $query = "SELECT * FROM NunesCarros WHERE id = :id";
        $stmt = $this->conn->prepare($query);
...
public function create() {
    try {
        $query = "INSERT INTO NunesCarros (marca, detalhes, foto) VALUES (:marca, :detalhes, :foto)";
        $stmt = $this->conn->prepare($query);
...
public function update() {
    try {
        ...
        $query = "UPDATE NunesCarros SET marca = :marca, detalhes = :detalhes, foto = :foto WHERE id = :id";
        $stmt = $this->conn->prepare($query);
...
public function delete($id) {
    try {
        $query = "DELETE FROM NunesCarros WHERE id = :id";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':id', $id, \PDO::PARAM_INT);
